...
architecture behave of func_control_s_axi is
    ...
    constant ADDR_M_DATA_0 : INTEGER := 16#10#;
    constant ADDR_M_CTRL   : INTEGER := 16#14#;
    constant ADDR_K_DATA_0 : INTEGER := 16#18#;
    constant ADDR_K_CTRL   : INTEGER := 16#1c#;
    constant ADDR_N_DATA_0 : INTEGER := 16#20#;
    constant ADDR_N_CTRL   : INTEGER := 16#24#;
    constant ADDR_X_DATA_0 : INTEGER := 16#28#;
    constant ADDR_X_DATA_1 : INTEGER := 16#2c#;
    constant ADDR_X_CTRL   : INTEGER := 16#30#;
    constant ADDR_Y_DATA_0 : INTEGER := 16#34#;
    constant ADDR_Y_DATA_1 : INTEGER := 16#38#;
    constant ADDR_Y_CTRL   : INTEGER := 16#3c#;
    constant ADDR_Z_DATA_0 : INTEGER := 16#40#;
    constant ADDR_Z_DATA_1 : INTEGER := 16#44#;
    constant ADDR_Z_CTRL   : INTEGER := 16#48#;
    ...
    signal int_z               : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_Z_DATA_1 =>
                        rdata_data <= RESIZE(int_z(63 downto 32), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_Z_DATA_1) then
                    int_z(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_z(63 downto 32));
                end if;
            end if;
        end if;
    end process;
