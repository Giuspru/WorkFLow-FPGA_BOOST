...
__attribute__((sdx_kernel("func", 0))) void func(unsigned int M, unsigned int K, unsigned int N, const ap_int<16> *x, const ap_int<16> *y, ap_int<32> *z);
# 2 "src/func.cc" 2

__attribute__((sdx_kernel("func", 0))) void func(unsigned int M, unsigned int K, unsigned int N, const ap_int<16> *x, const ap_int<16>* y, ap_int<32> *z){
#line 15 "/home/giuspru/HLS/tutorial-2023/a0.alveo_u55c_matrixmult/run_hls.tcl"
#pragma HLSDIRECTIVE TOP name=func
# 3 "src/func.cc"
...
#pragma hls interface mode=m_axi port=z offset=slave bundle=gmem
 ...
 VITIS_LOOP_8_1: for(int i=0; i<M ;i++){
        VITIS_LOOP_9_2: for(int j=0;j<N; j++){
#pragma hls pipeline
 ap_int<32> sum = 0;
            VITIS_LOOP_12_3: for(int k=0; k<K; k++){
                sum += x[i*K+k]*y[k*N+j];
            }
            z[i*N+j] = sum;
        }
